...
For Vue component props, add a `@hidden` JSDoc tag to keep an automatically populated prop out of the visual properties editor:

```ts [components/GeneratedCard.vue]
...
Required Vue props that have no value are marked with a red exclamation point on the component and beside the field label in the properties editor. Hidden props are excluded from this warning because they cannot be filled from the properties UI.
