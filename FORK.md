...
Copy `artifacts/nuxt-studio-1.7.0-pdf.1.tgz` into your application's `vendor/` directory, then run there:

```sh
pnpm add ./vendor/nuxt-studio-1.7.0-pdf.1.tgz
```
...
For several consuming projects, publish versioned builds under your own npm scope, then install with an alias such as `nuxt-studio@npm:@tasiotas/nuxt-studio@1.7.0-pdf.1`. That is a future distribution option; no npm package has been published. Avoid following a moving Git branch in production.
